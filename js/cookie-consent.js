...
required: false,
onAccept: function() {
// Render Wake-Up Ping (nur nach Einwilligung)
fetch(‘https://pdf-compressor-web.onrender.com/wakeup’, { mode: ‘no-cors’ }).catch(function() {});
  ...
  onReject: function() {
    // YouTube
    document.querySelectorAll('[id^="video-placeholder-"]').forEach(function(ph) {
      var num = ph.id.replace('video-placeholder-', '');
      var iframe = document.getElementById('video-iframe-' + num);
      ph.style.display = "flex";
      if (iframe) { iframe.style.display = "none"; iframe.src = ""; }
    });

    // PDF Kompressor
    var pdfPh = document.getElementById('pdf-placeholder');
    ...
    // Foto zu PDF
    var photoPh = document.getElementById('photo-placeholder');
    var photoIframe = document.getElementById('photo-iframe');
...
function loadNews() {
var placeholder = document.getElementById(‘news-placeholder’);
